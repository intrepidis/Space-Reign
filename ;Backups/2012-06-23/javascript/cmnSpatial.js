(function (global) {
	...
	CMN.spatial = (function () {
		...
		I.makeAddLineFunc = function (model, colour) {
		...
		I.sidesAtDepthWithinBounds = function (projector, camera, bounds) {
			...
			return new CMN.Bounds(
				CMN.misc.withinBounds(Math.floor(cornerO.x), 0, bounds.x),
				CMN.misc.withinBounds(Math.floor(cornerO.y), 0, bounds.y),
				CMN.misc.withinBounds(Math.floor(cornerP.x), 0, bounds.x),
				CMN.misc.withinBounds(Math.floor(cornerP.y), 0, bounds.y));
		};
