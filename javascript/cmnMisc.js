(function (global) {
    ...
    // jQuery extensions.
	(function ($) {
	    $.fn.disableSelection = function (shouldDisable) {
	        var cssRule;
	        shouldDisable = !(shouldDisable === false);
	        cssRule = shouldDisable ? 'none' : 'text';
	        return this.each(function () {
	            $(this).attr('unselectable', shouldDisable ? 'on' : 'off')
                       .css({
                           '-moz-user-select': cssRule,
                           '-webkit-user-select': cssRule,
                           'user-select': cssRule,
                           '-ms-user-select': cssRule
                       });
	        });
	    };
	})(jQuery);
	...
	CMN.misc = (function () {
		...
		(function EXPOSED_METHODS() {
			...
			X.getElementHtml = function($jQueryElement){
				return $jQueryElement.wrap('<p>').parent().html();
			};
			...
			X.createName = function () {
				...
				(function () {
					...
					nameLength = min + CMN.rand.nextWeightedInt(weights, weightsSum);
